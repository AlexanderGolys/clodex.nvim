...
The prompt creator keeps footer actions visible, docks the target-project picker on the left, preserves compatible drafts across kind switches, and can preview attached clipboard images in a separate pane. `C-S`, `C-Q`, `C-I`, and `C-L` plan, queue, run immediately, or send straight to the live chat; queue and chat submissions close the creator after success. Immediate direct execution currently works only with the Codex backend. Interactive Codex prompt dispatch sends the final submit keystroke with the prompt payload, while OpenCode keeps its delayed submit path.
