...
describe("clodex.app.queue_actions", function()
    ...
    it("retries starting the next queued item if the first post-reset dispatch fails", function()
        local original_defer_fn = vim.defer_fn
        vim.defer_fn = function(fn)
            fn()
            return 0
        ...
        assert.are.same({ "/new" }, sent)
        assert.are.equal(2, start_attempts)
        assert.are.equal(item.id, started_item_id)
    end)
    ...
    it("moves a history item back to queued when marked as not working", function()
        ...
        assert.is_true(moved.details:find("abcdef1234567890") ~= nil)
        assert.are.equal(nil, moved.history_summary)
        assert.are.same({ "abcdef1234567890" }, moved.history_commits)
        assert.are.equal(nil, moved.history_completed_at)
        assert.are.equal(1, refresh_count)
    end)
    ...
    it("flattens nested not-working prompts and keeps history coherent", function()
        ...
        assert.is_true(moved.details:find("`󰜘 22222222bbbbbbbb` `󰜘 11111111aaaaaaaa`", 1, true) ~= nil)
        assert.is_true(moved.details:find("second fix", 1, true) ~= nil)
        assert.is_true(moved.details:find("%- first fix") ~= nil)
        assert.are.same({ "22222222bbbbbbbb", "11111111aaaaaaaa" }, moved.history_commits)
