...
local function storage_path(storage_dir, session_file)
    ...
    return fs.join(storage_dir, ("%s-%s.json"):format(basename, digest))
end

local function legacy_sidecar_path(session_file)
    session_file = normalize_session_file(session_file)
    return session_file and (session_file .. LEGACY_SIDECAR_SUFFIX) or nil
end

local function resolve_state_path(storage_dir, session_file)
    local state_path = storage_path(storage_dir, session_file)
    if state_path and fs.is_file(state_path) then
        return state_path
    end

    local sidecar_path = legacy_sidecar_path(session_file)
    if sidecar_path and fs.is_file(sidecar_path) then
        return sidecar_path
    end
    return nil
end
...
function Persistence:save(app, session_file)
    local path = storage_path(self.storage_dir, session_file)
    if not path then
        return
    end
    fs.write_json(path, self:build_state(app))

    local legacy_sidecar = legacy_sidecar_path(session_file)
...
function Persistence:restore(app, session_file)
    local state_path = resolve_state_path(self.storage_dir, session_file)
