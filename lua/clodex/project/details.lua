...
local function normalize_root(project_root)
    return fs.normalize(project_root)
end

---@param root string
---@return string
local function metadata_id(root)
    return vim.fn.sha256(normalize_root(root)):sub(1, 16)
end
...
local function legacy_metadata_dir()
    return fs.join(vim.fn.stdpath("data"), "clodex", "workspaces", METADATA_DIRNAME)
end

---@param project_root string
---@return string
local function metadata_path(project_root)
    return fs.join(metadata_dir(), metadata_id(project_root) .. ".json")
end

---@param project_root string
---@return string
local function legacy_metadata_path(project_root)
    return fs.join(legacy_metadata_dir(), metadata_id(project_root) .. ".json")
end

---@param root string
---@return string[]
local function read_ignore_file(root, filename)
...
local function load_ignore_patterns(root)
    ...
    for _, filename in ipairs(IGNORE_FILES) do
        for _, pattern in ipairs(read_ignore_file(root, filename)) do
...
local function is_ignored(path, root, patterns)
    ...
    for _, pattern in ipairs(patterns) do
        if pattern:sub(-1) == "/" then
            local token = pattern:sub(1, -2)
            if rel_path == token or rel_path:match("^" .. token .. "/") or rel_path:match("/" .. token .. "/") then
                return true
            end
            if rel_path:match("/" .. token .. "$") then
                return true
            end
        elseif pattern:sub(1, 1) == "*" then
            local token = pattern:sub(2)
            if rel_path:match(token .. "$") or rel_path:match("/" .. token .. "$") then
                return true
            end
        ...
        elseif rel_path == pattern or rel_path:match("^" .. pattern .. "/") or rel_path:match("/" .. pattern .. "$") or rel_path:match("/" .. pattern .. "/") then
            return true
        end
    end
    return false
...
---@param project_root string
---@return table
local function read_metadata(project_root)
    local path = metadata_path(project_root)
    local legacy_path = legacy_metadata_path(project_root)
...
---@param project_root string
---@param metadata Clodex.ProjectDetails.Metadata
local function write_metadata(project_root, metadata)
    metadata.version = METADATA_VERSION
    fs.write_json(metadata_path(project_root), metadata)
end
...
local function load_metadata_snapshot(self, project_root)
    local metadata = read_metadata(project_root)
...
function Details:get_cached(project)
    ...
    cache_snapshot(self, project_root, snapshot, metadata.captured_at)
...
function Details:set_icon(project, icon)
    ...
    local clean_icon = type(icon) == "string" and vim.trim(icon) or ""
    next_snapshot.project_icon = clean_icon ~= "" and clean_icon or nil
    metadata.captured_at = os.time()
    metadata.snapshot = vim.deepcopy(next_snapshot)
    write_metadata(project_root, metadata)
    cache_snapshot(self, project_root, next_snapshot, metadata.captured_at)
end
...
function Details:delete(project_root)
    ...
    fs.remove(metadata_path(project_root))
end
...
local function list_files_from_tree(root, max_files)
    ...
    while #stack > 0 do
        ...
        for name, entry_type in vim.fs.dir(dir) do
            ...
            elseif entry_type == "file" and not is_ignored(path, root, ignore_patterns) then
                files[#files + 1] = path
                if #files >= max_files then
                    return files
                end
...
function Details:compute(project)
    local root = normalize_root(project.root)
    local files = list_files(root, MAX_SCANNED_FILES)
    local file_count = 0
    ...
    return {
        ...
        remote_name = git.remote_name(root),
        last_file_modified_at = last_file_modified_at,
        languages = language_profile:dominant_languages(language_totals),
    }
end
...
function Details:get(project)
    ...
    local metadata, existing = load_metadata_snapshot(self, project_root)
    ...
    write_metadata(project_root, metadata)
    cache_snapshot(self, project_root, snapshot, now)
    return vim.deepcopy(snapshot)
end
